 ...
 * @author sjke
 */
public class ChangePlayerRankAdminCommand implements BaseCommand {

	@Override
	public String call(ClientConnection connection, String message) {
		try {
			connection.getPlayer().setRank(Short.valueOf(message));
			//connection.sendPacket(new SM_ACCOUNT_INFO(connection.getPlayer(), connection.getAccount().getMoney()));
			return "Change rank for player [" + connection.getPlayer().getName() + "]";
		} catch (Exception e) {
			return "Command syntax : /adm player rank [0-54]";
		}
	}
