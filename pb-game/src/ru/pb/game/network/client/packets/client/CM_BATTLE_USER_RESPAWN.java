...
import ru.pb.game.network.client.packets.server.SM_BATTLE_USER_RESPAWN;
 ...
 * @author sjke
 */
public class CM_BATTLE_USER_RESPAWN extends ClientPacket {

	private int first, second, third, fourth, fifth, id, red, blue, head, beret, dino;

	public CM_BATTLE_USER_RESPAWN(int opcode) {
		super(opcode);
	}
			...
			public void run() {
				for (Player member : getConnection().getRoom().getPlayers().values()) {
					...
					if (slot.getState().ordinal() > 8)
						member.getConnection().sendPacket(new SM_BATTLE_USER_RESPAWN(getConnection().getRoom().getRoomSlotByPlayer(getConnection().getPlayer()).getId(), first, second, third, fourth, fifth, id, red, blue, head, beret, dino));
				}
			}
		});
	}
