...
import ru.pb.game.network.client.packets.server.SM_PROFILE_ENTER;
...
/**
 * Пакет входа в профиль
 *
 * @author sjke, DarkSkeleton
 */
public class CM_PROFILE_ENTER extends ClientPacket {

	public CM_PROFILE_ENTER(int opcode) {
		super(opcode);
	}
	...
	public void runImpl() {
		if (getConnection().getPlayer() != null) {
			...
			if (room != null) {
				room.getRoomSlotByPlayer(getConnection().getPlayer()).setState(SlotState.SLOT_STATE_INFO);
			...
			sendPacket(new SM_PROFILE_ENTER());
		}
	}
