...
public class PROTOCOL_BATTLE_PING_REQ extends ClientPacket {

	private byte[] bytes;

	public PROTOCOL_BATTLE_PING_REQ(int opcode) {
		super(opcode);
	}
	...
	public void runImpl() {
		...
		for (Player member : getConnection().getRoom().getPlayers().values()) {
			...
			if (slot.getState() == SlotState.SLOT_STATE_BATTLE) {
				member.getConnection().sendPacket(new PROTOCOL_BATTLE_PING_ACK(bytes));
			}
		}
	}
}